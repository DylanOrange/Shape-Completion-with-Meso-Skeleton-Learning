...
#        
        self.conv1_1 = torch.nn.Conv1d(512,512,1)#torch.nn.Conv1d(256,256,1) !
        self.conv1_2 = torch.nn.Conv1d(512,256,1)
        self.conv1_3 = torch.nn.Conv1d(256,int((self.crop_point_num)*3/128),1)   #16
        self.conv2_1 = torch.nn.Conv1d(128,12,1)#torch.nn.Conv1d(256,12,1) !
...
#        self.bn2_ = nn.BatchNorm1d(256)
    ...
    def forward(self,x):
        ...
        pc2_feat = F.relu(self.fc2_1(x_2))
        pc2_feat = pc2_feat.reshape(-1,128,64)
        pc2_xyz =self.conv2_1(pc2_feat) #12x64 center2
        
        pc3_feat = F.relu(self.fc1_1(x_1))
        pc3_feat = pc3_feat.reshape(-1,512,128)
        pc3_feat = F.relu(self.conv1_1(pc3_feat))
        pc3_feat = F.relu(self.conv1_2(pc3_feat)) # 256 128 
        pc3_xyz = self.conv1_3(pc3_feat) #48x128 fine
        ...
        pc2_xyz_expand = torch.unsqueeze(pc2_xyz,2)  #1 256 1 3
        pc3_xyz = pc3_xyz.transpose(1,2) # 1 128 48
        pc3_xyz = pc3_xyz.reshape(-1,256,8,3)
        pc3_xyz = pc2_xyz_expand+pc3_xyz
        pc3_xyz = pc3_xyz.reshape(-1,self.crop_point_num,3) 
