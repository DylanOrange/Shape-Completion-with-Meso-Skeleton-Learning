...
def random_scale_point_cloud_and_gt_new(input_data, skeleton_data, gt_data=None, scale_low=0.5, scale_high=2):
    """ Randomly scale the point cloud. Scale is per point cloud.
        Input:
            Nx3 array, original point cloud
        Return:
    ...
    scale = np.random.uniform(scale_low, scale_high)
    input_data[:, :3] *= scale
    skeleton_data[:, :3] *= scale
    if gt_data is not None:
        gt_data[:, :3] *= scale

    return input_data, skeleton_data, gt_data, scale
...
def shift_point_cloud_and_gt_new(input_data, skeleton_data, gt_data=None, shift_range=0.3):
    """ Randomly shift point cloud. Shift is per point cloud.
        Input:
          Nx3 array, original point cloud
        Return:
    ...
    shifts = np.random.uniform(-shift_range, shift_range, 3)
    input_data[:, :3] += shifts
    skeleton_data[:, :3] += shifts
    if gt_data is not None:
        gt_data[:, :3] += shifts
    return input_data, skeleton_data, gt_data
